---
lab:
  title: 更新 - Microsoft Foundry で Content Understanding の使用を開始する
...
1. Web ブラウザーで [Microsoft Foundry](https://ai.azure.com){:target="_blank"} (`https://ai.azure.com`) を開き、Azure の資格情報を使ってサインインします。 初めてサインインしたときに開かれるヒントまたはクイック スタートのペインを閉じます (また、**[ヘルプ]** ペインが開いている場合はそれも閉じます)。
...
7. 抽出されたフィールドが表示されている右側のペインで、**[結果]** タブを表示すると、JSON 形式の生の結果が表示されます。 **analyzerID** フィールドを確認します。このフィールドには、使用されたアナライザーの種類が含まれています。  
...
1. コマンドを実行すると、応答が JSON 形式で返されます。 分析は非同期で実行されるため、次のように応答には結果をポーリングして取得するのに使用できる **id** の値が含まれています。
...
試用した 3 つのアナライザー ("読み取り"、"レイアウト"、"ドキュメント フィールド") について考えてみましょう。これらは、Azure Content Understanding によって提供される多くの完全なアナライザーの 1 つです。****** 
