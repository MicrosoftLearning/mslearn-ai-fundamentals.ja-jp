...
1. プロジェクトに関する次の詳細情報を確認します。
    ...
    > \* キーベースの認証を禁止するポリシーを採用している企業または学校の Azure サブスクリプションを使用している場合、この演習ではチャット アプリを使用できません。 代わりに、別のブラウザー ベースのバージョンのアプリを [https://aka.ms/ask-anton](https://aka.ms/ask-anton){:target="_blank"} で入手できます。
