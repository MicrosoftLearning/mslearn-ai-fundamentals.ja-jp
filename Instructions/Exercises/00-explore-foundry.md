...
![Anton の画像。](./media/anton-icon.png)<br/>**こんにちは、Anton です。**<br/>このラボで、Microsoft Foundry プロジェクトを作成して探索し、AI アプリケーションをサポートするために作成できる資産を調べる手順を進められるよう、ヒントやコツを使ってサポートします。
...
1. Foundry ポータルのツール バーで **[ホーム]** を選択して Foundry ポータルのホーム ページに戻り、その後リソース一覧 (**[Microsoft Foundry]** ページ タイトルの横) でプロジェクトを選択します。
    ...
    > ![Anton の画像。](./media/anton-icon.png)<br/>**ヒント**: 最初にホーム ページに戻ったときに、親リソースがまだ選択されている可能性があります。 プロジェクトを選択することで、ポータルを使用してプロジェクト固有のアセットを扱えるようになります。
...
1. **[デプロイ]** ボタンを使用して、既定の設定を使用してモデルをデプロイします。 デプロイには 1 分ほどかかる場合があります。
    ...
    > ![Anton の画像。](./media/anton-icon.png)<br/>**ヒント**: モデルのデプロイにはリージョンのクォータが適用されます。 このモデルをプロジェクトの Azure リージョンにデプロイするのに十分なクォータがない場合は、別のチャット可能な *gpt* モデル (*gpt-5* や *gpt-5.1* など) をお使いください。 別の方法として、新しいプロジェクトを別のリージョンに作成することもできます。
...
1. プレイグラウンドで、モデル デプロイが選択されていることを確認します。
    ...
    > ![Anton の画像。](./media/anton-icon.png)<br/>**ヒント**: モデル デプロイの名前を覚えておいてください。 この情報は後で必要になります。
...
1. プロジェクトに関する次の詳細情報を確認します。
    ...
    > ![Anton の画像。](./media/anton-icon.png)<br/>**ヒント**: キーベースの認証を禁止するポリシーを持つ企業または学校の Azure サブスクリプションを使用している場合は、Entra ID 認証を使用 "できます" が、テナントにアプリを登録する必要があります (グローバル管理者のアクセス許可が必要です)。** 最終手段として、代替のブラウザーベース (Azure 以外) のバージョンのアプリを [https://aka.ms/computing-history-browser](https://aka.ms/computing-history-browser) {:target="_blank"} で入手できます。
...
1. 2 つ目のブラウザー タブを開き、`https://aka.ms/computing-history-foundry` にある [コンピューティング履歴エージェント](https://aka.ms/computing-history-foundry){:target="_blank"} アプリに移動します。
    ...
    コンピューティング履歴アプリが開き、**[構成]** パネルが展開されます (展開されていない場合は、チャット ペインの上部にある矢印を使用して展開します)。
...
1. 抽出した他のコンピューターの画像も試してみてください。
    ...
    > ![Anton の画像。](./media/anton-icon.png)<br/>**ヒント**: [Bing](https://www.bing.com/images/search?q=vintage+computers){:target="_blank"} で、ヴィンテージ コンピューターの独自の画像を検索することもできます。
...
1. 抽出した他の PCB の画像を試して、それらが発生した可能性のあるコンピューターの種類の特定に、エージェントが役立つかどうかを確認します。

    > ![Anton の画像。](./media/anton-icon.png)<br/>**ヒント**: [Bing](https://www.bing.com/images/search?q=vintage-computer-component-serial-numbers){:target="_blank"} で独自の画像を検索することもできます。 特定のヴィンテージ コンピューターからシリアル番号ラベルを検索してみる
