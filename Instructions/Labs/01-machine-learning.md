...
>**重要** 現時点では、Azure Machine Learning スタジオでモデルを Web サービスとして作成してデプロイできますが、Studio でテストすることはできません。 そのため、クリーンアップ前の手順の最後のセクション以外のすべてを完了できます。 この変更については、随時お知らせします。
   ...
   **タスクの種類とデータ**:
    ...
    **タスク設定**:
    ...
    - **制限**: "このセクションを展開します"**
        ...
        - **同時試行の最大数**: 3
        - **最大ノード**: 3
        - **[Metric score threshold]\(メトリック スコアしきい値\)**: 0.085 (*モデルで、正規化された二乗平均平方根誤差メトリック スコアが 0.085 以下を達成した場合にジョブが終了するようにします。*)
        - **タイムアウト**: 15
        - **Iteration timeout \(反復のタイムアウト\)**:15
...
>**重要** 現在、Azure Machine Learning スタジオでは、デプロイ テストを使用するために必要なデータセットの種類の作成はサポートされていません。 解決策については、随時お知らせします。 
